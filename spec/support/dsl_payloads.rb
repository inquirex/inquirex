...
module DslPayloads
  ...
  def rich_flows
    {
      ...
      "placeholders in text"    => placeholder_text_flow,
      "send_email inline form"  => email_inline_flow,
    ...
    <<~'RUBY'
      Inquirex.define id: "waitlist" do
        ...
        send_email do
          ...
          markdown_text "**{{name}}** <{{email}}> just finished the form."
        end
      end
    RUBY
  end

  # `{{field}}` placeholders in user-facing strings. They are ordinary text to
  ...
  # `#{}` stays forbidden; see the "string interpolation" payload.
  #
  # The syntax here is the gem's own ({Inquirex::Template}): a flat key
  # resolved against `Answers#to_flat_h`, no namespace prefix and no filters.
  # Liquid's `{{ answers.name }}` / `{{ price | round: 2 }}` would pass
  # validation just as happily — every `{{ }}` is inert text to Prism — but
  # would render to an empty string and to raw template text respectively, so
  # writing it that way here would document a convention the gem does not
  # implement.
  #
  # Note also that the gem renders `question` / `text` nowhere today: only
  # `SendEmail#to_mail` calls the renderer. A placeholder in a question is
  # inert unless the *host* chooses to render it. This flow asserts what the
  # validator does with such a string, not what any renderer does.
  #
  # @return [String]
  def placeholder_text_flow
    <<~'RUBY'
      Inquirex.define do
        accumulator :price, type: :currency, default: 0
        start :name
        ...
        ask :email do
          type :email
          question "Thanks {{name}}, where can we reach you?"
          transition to: :quote
        end

        say :quote do
          text "{{name}}, your estimate is ${{price}}."
        end
      end
    RUBY
  end

  # The inline keyword form of send_email, with multi-line heredoc bodies.
  # Prism parses a dedented heredoc as an InterpolatedStringNode whose parts
  # are all plain StringNodes, so rejecting that node class outright would
  # break this — the reason Validator#static_string? inspects the parts
  # instead.
  #
  # @return [String]
  def email_inline_flow
    <<~RUBY
