...
DSL_SIGNATURES = {
  ...
  email:  ->(name) { Inquirex::DSL::SendEmailBuilder.instance_method(name) },
...
RSpec.describe Inquirex::SafeSource::Vocabulary do
      ...
      V.allowed_names(scope).each do |name|
        describe "`#{name}`" do
          ...
          it "opens a block only into a registered scope" do
            # #block_scope unwraps the `{ optional: scope }` form, so an
            # optional block is held to the same standard as a required one.
            expect(V.scope?(spec.block_scope)).to be(true) if spec.block_scope
          end
        end
      end
    end
  ...
  describe "the excluded words" do
    ...
    # `action`, `run`, `webhook` and `allowed_domains` used to be excluded
    # here. They were deleted from the gem instead, which is a stronger
    # guarantee than refusing them in safe mode: there is no `unsafe: true`
    # that brings them back. There is consequently no :action scope at all,
    # and the words read as unknown rather than as deliberate exclusions.
    it "has no action scope left to exclude anything in" do
      expect(described_class.excluded_names(:action)).to be_empty
      expect(described_class.allowed_names(:action)).to be_empty
      expect(described_class.allowed_names(:flow)).not_to include(:action, :allowed_domains)
    end
  end

  describe "the top-level send_email verb" do
    it "opens an optional email block, and takes no positional arguments in either form" do
      spec = described_class.spec_for(:flow, :send_email)

      # Optional because the same fields may be given inline as keywords.
      expect(spec.block).to eq(optional: :email)
      expect(spec.positional).to be_empty
    end

    it "gates on a rule, so `if:` cannot smuggle in an expression" do
      expect(described_class.spec_for(:flow, :send_email).keywords[:if]).to eq(:rule)
    end

    it "describes the same fields in the inline form as the block form" do
      inline = described_class.spec_for(:flow, :send_email).keywords.keys - [:if]

      expect(inline).to match_array(described_class.allowed_names(:email))
    end

    it "allows exactly the setters SendEmailBuilder implements" do
      expect(described_class.allowed_names(:email))
        .to match_array(Inquirex::DSL::SendEmailBuilder.public_instance_methods(false) - described_class::PLUMBING)
    end

    it "accepts only static values, so no Ruby can reach a template" do
      kinds = described_class.allowed_names(:email).map { |name| described_class.spec_for(:email, name).positional }

      # Bodies and addresses are single strings; `headers` alone is a literal
      # Hash. Neither shape admits an expression.
      expect(kinds.uniq).to contain_exactly(%i[string], %i[literal])
    end
  end
